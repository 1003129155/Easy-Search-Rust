...
fn build_plugin_router() -> Router {
    ...
    router.register(Box::new(plugin_calculator::CalculatorPlugin));
    router.register(Box::new(plugin_url::UrlPlugin));
    router.register(Box::new(plugin_web_search::WebSearchPlugin::default()));
    router.register(Box::new(plugin_bookmark::BookmarkPlugin::new()));
    router.register(Box::new(plugin_program::ProgramPlugin::new()));
    router.register(Box::new(plugin_sys_cmd::SysCmdPlugin));
    router.register(Box::new(plugin_shell::ShellPlugin::new()));
    router.register(Box::new(plugin_process_killer::ProcessKillerPlugin::new()));
    router.register(Box::new(plugin_win_settings::WinSettingsPlugin));
    router
}
...
fn show_window_safe(hwnd: HWND) {
    ...
    // Get the item count while briefly borrowing
    let item_count = APP_STATE.with(|state| {
        state.borrow().as_ref().map_or(0, |app| app.items.len())
    });
    ...
    unsafe {
        ...
        crate::log!("show_window_safe: SetForegroundWindow");
...
fn show_window(app: &mut AppState) {
    unsafe {
        ...
        crate::log!("show_window: SetForegroundWindow");
...
fn do_render() {
    APP_STATE.with(|state| {
        ...
        if let Some(ref mut app) = *state {
            ...
            // Render preview panel if we have preview info and results
            if let Some(ref preview) = app.preview {
                if !app.items.is_empty() {
                    let results_height = layout::window_height(app.items.len());
                    app.renderer.render_preview(preview, results_height);
                }
            }
        }
    });
