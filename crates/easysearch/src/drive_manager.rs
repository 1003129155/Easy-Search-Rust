...
//! Per-drive index manager for `easysearch`.
//!
//! Indexes are built by the background worker (see `main.rs`) and installed
//! here once ready. The manager also holds the USN cursor for each drive and
//! applies incremental events produced by the poll loop.
...
impl DriveManager {
    ...
    pub(crate) fn indexes(&self) -> impl Iterator<Item = &EsIndex> {
    ...
    #[must_use]
    pub(crate) fn drive_labels(&self) -> Vec<String> {
        self.indexes
            .iter()
    ...
    pub(crate) fn install(&mut self, drive_letter: char, index: EsIndex) {
...
fn build_index_windows(letter: char, cache_dir: Option<&Path>) -> Result<EsIndex, String> {
            ...
            match read_flow_cache(dir, volume_serial) {
                Ok(Some(cached)) => match query_usn_journal(drive) {
                    Ok(info) => {
                        ...
                        eprintln!(
                            "[easysearch] {letter}: journal id changed (cache {} vs live {}), rebuilding",
                            cached.status.journal_id, info.journal_id
                        );
                    }
                ...
                Ok(None) => {}
                Err(err) => {
                    eprintln!("[easysearch] cache invalid for {letter}: ({err}) — rebuilding");
                }
            }
        }
    }
    ...
    match query_usn_journal(drive) {
        ...
        Err(err) => {
            eprintln!("[easysearch] {letter}: query_usn_journal failed: {err}");
        }
    }
    ...
    if let Some(dir) = cache_dir {
        if let Some(volume_serial) = probe_volume_serial(letter) {
            ...
            {
                eprintln!("[easysearch] cache write failed for {letter}: {err}");
            }
        }
    }
...
fn build_index_from_mft_index(
    ...
    for record in &mft_index.records {
        ...
        drafts.push(RowDraft { frs, parent_frs, name, flags: ff });
    }
    ...
    let root_di = lookup_draft(root_frs).unwrap_or(0);
    {
        ...
        let flow_idx = builder.add_record(d.frs, u32::MAX, &d.name, d.flags, 0)
    ...
    while inserted != prev_inserted {
        ...
        for di in 0..drafts.len() {
            ...
            let d = &drafts[di];
            let parent_flow = resolve_parent(d.parent_frs, d.frs, no_parent, &lookup_draft, &draft_to_flow);
            if parent_flow != u32::MAX || d.parent_frs == no_parent || d.parent_frs == d.frs {
                let fi = builder.add_record(d.frs, parent_flow, &d.name, d.flags, 0)
                    .map_err(|e| e.to_string())?;
                draft_to_flow[di] = fi;
                inserted += 1;
            }
    ...
    for di in 0..drafts.len() {
        if draft_to_flow[di] == u32::MAX {
            let d = &drafts[di];
            let fi = builder.add_record(d.frs, root_flow, &d.name, d.flags, 0)
                .map_err(|e| e.to_string())?;
            draft_to_flow[di] = fi;
        }
    }
